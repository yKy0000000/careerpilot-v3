...
| 结论 | 数字 | 参照物 |
|---|---|---|
| 逐项缺口诊断可靠 | gaps recall 0.66~0.75 | 等预算瞎猜基线 0.32~0.36 的 **2 倍** |
...
| 层级 | 病灶 | 后果 |
|---|---|---|
| L9 | gaps 池化：30 条 JD 并成大集合 | recall 虚高 0.94（真值 0.66~0.75） |
...
docs/        大纲 + L10 执行流程 + 复核记录
reports/     阶段报告（L9 / L10 / L11 终稿）
```

> `results/`（评测原始结果）与个人学习库本地生成、不入库，可在本地用 `eval.run` 重新生成。
...
Python · FastAPI · openai SDK 3.0.0 · LangChain · SQLModel(SQLite) · sentence-transformers(bge-small-zh-v1.5) · numpy · scipy（配对检验）· httpx · trafilatura
